...
@section('content')
    ...
    <div class="max-w-3xl mx-auto mt-10 bg-white shadow-md rounded-lg p-8 border">
        <div class="text-center text-xl font-bold mb-12">Edit Data Pengguna</div>
        @if ($errors->any())
    ...
    <form method="POST" action="{{ route('admin.users.update', $editUser->nim) }}">
        @csrf
            ...
            <div class="mb-6">
                <label for="nama" class="block font-semibold mb-4">Nama :</label>
                <input type="text" name="nama" value="{{ old('nama', $editUser->nama) }}"
                    class="w-full text-sm border border-gray-300 bg-gray-50 rounded-lg px-4 py-2">
            </div>

            <div class="mb-6">
                <label for="email" class="block font-semibold mb-4">Email :</label>
                <input type="email" name="email" id="email" value="{{ old('email', $editUser->email) }}"
                    class="w-full text-sm border border-gray-300 bg-gray-50 rounded-lg px-4 py-2">
            </div>

            <div class="mb-12">
                <label for="nim" class="block font-semibold mb-4">Nomor Induk :</label>
