<x-form>
    ...
    <div class="w-1/2 px-48 place-content-center">
        ...
        <form method="POST" action="/register">
            @csrf
            <div>
                <label for="name" class="block my-3 text-sm font-medium text-gray-900">Nama Lengkap</label>
                <input id="name" type="text" name="name" required value="{{ old('name') }}"
                    class="mb-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
            ...
            <div>
                <label for="nim" class="block my-3 text-sm font-medium text-gray-900">Nomor Induk
                    Mahasiswa/Pegawai</label>
                <input id="nim" type="text" name="nim" required value="{{ old('nim') }}"
                    class="mb-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg
                           focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                @error('nim')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
